...
class UndecidableWarning(UserWarning):
    pass
...
class ComputableType(type):
    class RationalType(type):
        ...
        def _special_sub(cls,numerator_left:int,denominator_left:int,numerator_right:int,denominator_right:int)->tuple[int,int,bool]:
            if denominator_left==0 and denominator_right==0:
                if numerator_left>0 and numerator_right<0:
                    return 1,0,True
                if numerator_left<0 and numerator_right>0:
                    return -1,0,True
        ...
        def _analyze_input_for_one_argument(cls,arg:RationalLike)->tuple[int,int,bool]:
            ...
            if isinstance(arg,family_root):
                return arg.numerator,arg.denominator,False
            if isinstance(arg,int):
                return arg,1,True
            if isinstance(arg,Fraction):
                return arg.numerator,arg.denominator,True
        ...
        def _analyze_input(cls,*args:RationalLike)->tuple[int,int,bool]:
            if len(args)==1:
                return cls._analyze_input_for_one_argument(args[0])
            if len(args)==2:
                up,down=args
        ...
        def __dir__(cls):
            names=super().__dir__()
            return [name for name in names if name not in type(cls)._unqueryable_attribute]
        def __setattr__(cls,name,value):
            mcls=type(cls)
            if name in mcls._unwritable_attribute:
                if name in mcls._unreadable_attribute:
                    raise AttributeError("The class attribute cannot be assigned a value")
        ...
        @property
        def cache_keys(cls)->list[IntegerRatio]:
    ...
    class RealType(type):
        ...
        def __setattr__(cls,name,value):
            ...
            if name=='set_max_denominator_for_hash':
                ...
                if max_denominator is not None:
                    if value!=max_denominator:
                        raise ValueError(f"{cls.__name__}.set_max_denominator_for_hash can be set at most once")
                    else:
                        return
                elif not isinstance(value,int):
                    raise TypeError(f"Expected an integer, type of input is {type(value)}")
                if value<1:
                    raise ValueError(f"Expected a positive integer, input is {value}")
            super().__setattr__(name,value)
        def __delattr__(cls,name):
            mcls=type(cls)
            if name in mcls._undeletable_attribute:
        ...
        def _analyze_input_for_sign_function(cls,arg)->tuple[IntegerRatio|SignFunction,bool]:
            try:
                ...
                if denominator_input==0:
                    raise ValueError('The input must be a finite number.')
                return (numerator_input,denominator_input),True
            except TypeError:
                if isinstance(arg,Callable) and not isinstance(arg,type):
                    return arg,False
        ...
        def __call__[T](cls:type[T],*args,**kwargs)->T:
            ...
            if args_length==0:
                try:
                    sign_function=kwargs.pop('sign_function')
                except KeyError:
                    raise TypeError("Expected a sign_function")
                ...
                raise TypeError("Expected at most five arguments")
            is_true_type1,is_true_type2=isinstance(is_possible_rational,bool),isinstance(is_possible_irrational,bool)
            if not is_true_type1:
                raise TypeError(f"Expected a bool object, input is {type(is_possible_rational)}")
            if not is_true_type2:
                raise TypeError(f"Expected a bool object, input is {type(is_possible_irrational)}")
            if not is_possible_rational and not is_possible_irrational:
                raise ValueError('The input has a contradiction')
            ...
            if is_exactly_rational:
                if not is_possible_rational:
                    raise ValueError('The input has a contradiction')
                if is_possible_irrational:
                    is_possible_irrational=False
                ...
                return instance
            if is_possible_rational and is_possible_irrational:
                warn_undecidable()
            instance=cls.__new__(cls)
            instance._is_called=True
            ...
            for other_category in number_class_list:
                if category is not other_category:
                    ...
                    if number_class_name.endswith('Number'):
                        ...
                        raise ValueError("The name of number class must end with 'Number'")
...
class ComputableNumber(metaclass=ComputableType):
    ...
    class RationalNumber[R:Real](metaclass=ComputableType.RationalType):
        ...
        def __setattr__(self,name,value):
            ...
            if self._is_frozen:
                raise ValueError('This property setting is invalid because the object is frozen')
            numerator_v,denominator_v,_=cls._analyze_input_for_one_argument(value)
            if name=='numerator':
                self._inplace_div(numerator_v,denominator_v,self.denominator,1)
            else:
        ...
        def _inplace_setting(self,numerator:int,denominator:int,is_special:bool)->None:
            if self.numerator==numerator and self.denominator==denominator:
                return
            property_setter=super().__setattr__
            property_setter('numerator',numerator)
            property_setter('denominator',denominator)
        ...
        def _inplace_return(self,numerator:int,denominator:int,is_special:bool)->Self:
            if not self._is_frozen:
                self._inplace_setting(numerator,denominator,is_special)
                return self
            cls=type(self)
            if is_special:
                return cls._construct_new_object_for_simple(numerator,denominator)
        ...
        def simplify(self)->None:
            ...
            if denominator==0:
                if numerator>0:
                    property_setter('numerator',1)
                elif numerator<0:
                    property_setter('numerator',-1)
            elif numerator==0:
                property_setter('denominator',1)
            else:
                numerator,denominator=cls._simplify(numerator,denominator)
                property_setter('numerator',numerator)
                property_setter('denominator',denominator)
        ...
        def intern(self)->Self:
            ...
            if self._hash is None:
                hash_value=cls._compute_hash(*key_tuple)
                property_setter('_hash',hash_value)
            property_setter('_is_frozen',True)
        ...
        def safe_setting(self,name:str,value)->Self:
            ...
            if self._is_frozen:
                if name=='numerator':
                    numerator,denominator,is_special=cls._special_div(numerator_v,denominator_v,self.denominator,1)
                else:
                    numerator,denominator,is_special=cls._special_div(self.numerator,1,numerator_v,denominator_v)
                if is_special:
                    return cls._construct_new_object_for_simple(numerator,denominator)
                else:
                    return cls._construct_new_object_for_straight(numerator,denominator)
        ...
        def __repr__(self):
            ...
            if denominator==0:
                if numerator>0:
                    return f'{head_str}.infty'
                if numerator<0:
                    return f'{head_str}.minfty'
                return f'{head_str}.nan'
            if numerator==0:
                return f'{head_str}.ZERO'
            if numerator==denominator:
                return f'{head_str}.ONE'
        ...
        def _check_overflow_for_int(self)->None:
            if self.denominator==0:
                if self.numerator>0:
                    raise OverflowError(f'cannot convert {type(self)} Infinity to integer')
                if self.numerator<0:
                    raise OverflowError(f'cannot convert {type(self)} -Infinity to integer')
        ...
        def __abs__(self):
            ...
            cls=type(self)
            if self._is_simplified:
                return cls._construct_new_object_for_simple(-self.numerator,self.denominator)
        ...
        def _round_half_to_even(numerator:int,denominator:int)->int:
            ...
            two_remainder<<=1
            if two_remainder>denominator:
                quotient+=1
            elif two_remainder==denominator and quotient&1:
                quotient+=1
            return quotient
        def __round__(self,n:int=None)->int|Self:
            ...
            if n is None:
                self._check_overflow_for_int()
                return cls._round_half_to_even(self.numerator,self.denominator)
            if self.denominator==0:
                if self.numerator>0:
                    return cls.infty.__copy__()
                if self.numerator<0:
        ...
        def rational_bound(self,max_denominator:int=1)->tuple[Self,Self]:
            ...
            if max_denominator<1:
                raise ValueError('max_denominator must be greater than or equal to 1') 
            self.simplify()
        ...
        def rational_round(self,max_denominator:int=1)->Self:
            ...
            if max_denominator==1:
                ...
                two_remainder<<=1
                if two_remainder>self.denominator:
                    quotient+=1
                elif two_remainder==self.denominator and quotient&1:
                    quotient+=1
        ...
        def _simplest_rational_in_interval(numerator_left:int,denominator_left:int,numerator_right:int,denominator_right:int)->IntegerRatio:
            ...
            if cross_product_diff==1:
                return numerator_left+numerator_right,denominator_left+denominator_right
        ...
        def simplest_rational_in_interval(self,other:Self)->Self:
            ...
            if not isinstance(other,root_cls):
                raise TypeError(f'other must be an instance of {root_cls.__name__}')
            if self.denominator==0 or other.denominator==0:
                raise ValueError('Cannot find simplest rational in interval including infinity or NaN')
            cross_product_diff=other.numerator*self.denominator-self.numerator*other.denominator
            if cross_product_diff<=0:
                raise ValueError('The precondition is not satisfied: self < other')
            left_integer=self.numerator//self.denominator
            right_integer=-(-other.numerator//other.denominator)
            if left_integer+1<right_integer:
                raise ValueError('The precondition is not satisfied: floor(self) + 1 == floor(other)')
            if not self._is_simplified:
                self.simplify()
            if not other._is_simplified:
                other.simplify()
        ...
        def _input_iterator(cls,*args:RationalLike)->Iterator[IntegerRatio]:
            ...
            if len(args)==1:
                arg=args[0]
                try:
                    numerator,denominator,_=cls._analyze_input_for_one_argument(arg)
                    yield numerator,denominator
                except TypeError:
                    for item in arg:
                        numerator,denominator,_=cls._analyze_input_for_one_argument(item)
                        yield numerator,denominator
            else:
                for arg in args:
                    numerator,denominator,_=cls._analyze_input_for_one_argument(arg)
                ...
                if denominator==0:
                    if numerator>0:
                        for re_numerator,re_denominator in iter_loop:
                            if re_denominator==0 and re_numerator<=0:
                                return cls.nan.__copy__()
                        return cls.infty.__copy__()
                    elif numerator<0:
                        for re_numerator,re_denominator in iter_loop:
        ...
        def _iroot_rational(cls,numerator:int,denominator:int,degree:int)->tuple[IntegerRatio,IntegerRatio,bool]:
            ...
            if not is_numerator_exact:
                ...
                numerator_right=numerator_root
            if not is_denominator_exact:
                is_exact=False
                denominator_right=denominator_root+1
            else:
                denominator_right=denominator_root
        ...
        def iroot(self,degree:int)->tuple[Self|None,bool]:
            ...
            numerator,denominator=self.as_integer_ratio()
            if numerator<0:
                raise ValueError('Cannot compute root of negative rational')
            if denominator==0:
                if numerator>0:
                    return cls.infty.__copy__(),True
                return cls.nan.__copy__(),False
            answer_left,answer_right,is_exact=cls._iroot_rational(numerator,denominator,degree)
            if is_exact:
                return cls._construct_new_object_for_straight(*answer_left),True
            return None,False
        @classmethod
        ...
        def __pow__(self,exponent:RationalLike)->Self|R|NotImplementedType:
            ...
            try:
                numerator_exponent,denominator_exponent,is_simple=cls._analyze_input_for_one_argument(exponent)
            except TypeError:
                return NotImplemented
            ...
            if result is not None:
                return result
            if not is_simple:
                numerator_exponent,denominator_exponent=cls._simplify(numerator_exponent,denominator_exponent)
        ...
        def __rpow__(self,base:RationalLike)->Self|R|NotImplementedType:
            ...
            try:
                numerator_base,denominator_base,is_simple=cls._analyze_input_for_one_argument(base)
            except TypeError:
                return NotImplemented
            result=cls._check_input_for_pow(numerator_base,denominator_base,numerator_exponent,denominator_exponent)
            if result is not None:
                return result
            if not is_simple:
                numerator_base,denominator_base=cls._simplify(numerator_base,denominator_base)
            return cls._pow_helper(numerator_base,denominator_base,numerator_exponent,denominator_exponent)
        __ipow__=__pow__
        @classmethod
        ...
        def logarithm(cls,base:RationalLike,argument:RationalLike)->Self|R:
            ...
            if denominator_base==0 or denominator_argument==0:
                raise ValueError('base and argument must be finite')
            if numerator_base<=0:
                raise ValueError('base must be positive')
            if numerator_argument<=0:
                raise ValueError('argument must be positive')
            if numerator_base==denominator_base:
                raise ValueError('base must not be 1')
            if numerator_argument==denominator_argument:
                return cls.ZERO
            if not base_is_simple:
                numerator_base,denominator_base=cls._simplify(numerator_base,denominator_base)
            if not argument_is_simple:
                numerator_argument,denominator_argument=cls._simplify(numerator_argument,denominator_argument)
            ...
            if base_primitive==argument_primitive:
                return cls._construct_new_object_for_straight(argument_degree,base_degree)
            if (argument_primitive[1],argument_primitive[0])==base_primitive:
                return cls._construct_new_object_for_straight(-argument_degree,base_degree)
            if numerator_base<denominator_base:
                outside_sign=-1
                numerator_base,denominator_base=denominator_base,numerator_base
    ...
    class RealNumber[Q:Rational](metaclass=ComputableType.RealType):
        ...
        def _int_bound_for_init(self,left:RationalLike=None,right:RationalLike=None)->tuple[int,int]|Q:
            ...
            if left is not None:
                ...
                if denominator_left==0:
                    if numerator_left<0:
                        left=None
                    elif numerator_left>0:
                        raise ValueError('The left endpoint cannot be positive infinity')
                    else:
                        raise ValueError('The left endpoint cannot be NaN')
            if right is not None:
                ...
                if denominator_right==0:
                    if numerator_right>0:
                        ...
                        raise ValueError('The right endpoint cannot be negative infinity')
                    else:
                        raise ValueError('The right endpoint cannot be NaN')
            if left is None:
                if right is None:
                    ...
                    if compare_result==-1:
                        ...
                        while compare_result==-1:
                            ...
                            compare_result=self._init_sign_func(right_integer,1)
                        if compare_result==0:
                            return Rationalclass(right_integer,1)
                        ...
                        while compare_result==1:
                            ...
                            compare_result=self._init_sign_func(left_integer,1)
                        if compare_result==0:
                            return Rationalclass(left_integer,1)
                    else:
                        return Rationalclass.ZERO
                else:
                    ...
                    if compare_result==1:
                        ...
                        return Rationalclass(numerator_right,denominator_right)
                    else:
                        raise ValueError('The right endpoint must be greater than or equal to the real number')
            else:
                if right is None:
                    ...
                    if compare_result==-1:
                        ...
                        return Rationalclass(numerator_left,denominator_left)
                    else:
                        raise ValueError('The left endpoint must be less than or equal to the real number')
                else:
                    ...
                    if right_diff_left>0:
                        ...
                        if compare_result_left==-1:
                            ...
                            if compare_result_right==1:
                                left_integer=numerator_left//denominator_left
                                right_integer=-(-numerator_right//denominator_right)
                            elif compare_result_right==0:
                                return Rationalclass(numerator_right,denominator_right)
                            else:
                                raise ValueError('The right endpoint must be greater than or equal to the real number')
                        elif compare_result_left==1:
                            raise ValueError('The left endpoint must be less than or equal to the real number')
                        else:
                            compare_result_right=self._init_sign_func(numerator_right,denominator_right)
                            if compare_result_right==1:
                                ...
                                raise ValueError('The left and right endpoints are equal to the real number but they are not equal')
                            else:
                                raise ValueError('The right endpoint must be greater than or equal to the real number')
                    elif right_diff_left==0:
                        if self._init_sign_func(numerator_left,denominator_left)==0:
                            return Rationalclass(numerator_left,denominator_left)
                        else:
                            raise ValueError('The left and right endpoints are equal but not equal to the real number')
                    else:
                ...
                if denominator_right>max_denominator:
                    ...
                    while True:
                        ...
                        if denominator_small<=max_denominator:
                            ...
                            if small_is_left:
                                return constructor(numerator_small,denominator_small),constructor(numerator_large,denominator_large)
                            else:
                                return constructor(numerator_large,denominator_large),constructor(numerator_small,denominator_small)
                else:
                ...
                if denominator_mid<=max_denominator:
                    ...
                    while True:
                        ...
                        if compare_result==-1:
                            ...
                            return self._exact_rational,self._exact_rational
                else:
                    return left_rational,right_rational
        def __init__(self,*args):
            ...
            if args_length==1:
                ...
                if not self._is_called and is_possible_rational and is_possible_irrational:
                    warn_undecidable()
        ...
        def current_bound(self,depend_on_structure:bool=False)->tuple[Q,Q]:
            if not depend_on_structure:
                return self._nearest_left,self._nearest_right
            else:
                return self._left_rational,self._right_rational
        def current_width(self,depend_on_structure:bool=False)->Q:
            if not depend_on_structure:
                return (self._nearest_right-self._nearest_left).intern()
            else:
                if not self._is_possible_irrational:
                    return type(self)._Rational.ZERO
                else:
        ...
        def as_integer_ratio(self,fallback_max_denominator:int=None)->IntegerRatio:
            if fallback_max_denominator is None:
                if not self._is_possible_rational:
                    raise ValueError('This number is exactly irrational.')
                if self._is_possible_irrational:
                    raise ValueError('This number has no rational representation yet.')
                return self._exact_rational.as_integer_ratio()
            if not isinstance(fallback_max_denominator,int):
                raise TypeError('fallback_max_denominator must be an integer')
            if fallback_max_denominator<1:
                raise ValueError('fallback_max_denominator must be positive')
            if not self._is_possible_irrational:
                return self._exact_rational.as_integer_ratio()
            ...
            compare_result=self._init_sign_func(numerator_mid,denominator_mid)
            if compare_result==-1:
                return numerator_right,denominator_right
            if compare_result==1:
        ...
        def float_bound(self)->tuple[float,float]:
            ...
            if not self._is_possible_irrational:
                self._float_bound=self._exact_rational.float_bound()
                return self._float_bound
            left_integer=self._floor
            if left_integer>=0:
                abs_floor=left_integer
                def abs_sign_func(x,y):
                    return self._init_sign_func(x,y)
                is_positive=True
            else:
                abs_floor=-(left_integer+1)
                def abs_sign_func(x,y):
                    return -(self._init_sign_func(-x,y))
                is_positive=False
            if abs_floor>=1:
                max_int=((1<<53)-1)<<971
                if abs_floor>=max_int:
                    abs_float_bound=(float(max_int),float('inf'))
                else:
                    ...
                    if exponent>=0:
                        ...
                        while search_left+1!=search_right:
                            ...
                            if compare_result==1:
                                ...
                                result=search_mid/denominator_scalar
                                abs_float_bound=(result,result)
                                is_found=True
                                break
                        if not is_found:
                            abs_float_bound=(search_left/denominator_scalar,search_right/denominator_scalar)
            else:
                test_min_denominator=1<<1074
                compare_result=abs_sign_func(1,test_min_denominator)
                if compare_result==1:
                    abs_float_bound=(float(0),1/test_min_denominator)
                elif compare_result==-1:
                    if is_positive:
                        numerator_left,denominator_left=self._nearest_left.numerator,self._nearest_left.denominator
                        numerator_right,denominator_right=self._nearest_right.numerator,self._nearest_right.denominator
                        ...
                        numerator_right,denominator_right=-(self._nearest_left.numerator),self._nearest_left.denominator
                    left_invert_ceil=-(-denominator_left//numerator_left)
                    if left_invert_ceil&(left_invert_ceil-1)==0:
                        exponent_left=left_invert_ceil.bit_length()-1
                    else:
                        exponent_left=left_invert_ceil.bit_length()
                    ...
                    while exponent_left-1!=exponent_right:
                        ...
                        if compare_result==1:
                            exponent_right=exponent_mid
                            right_denominator=test_denominator
                        elif compare_result==-1:
                            exponent_left=exponent_mid
                            left_denominator=test_denominator
                        else:
                            result=1/test_denominator
                            abs_float_bound=(result,result)
                            is_found=True
                            break
                    if not is_found:
                    ...
                    result=1/test_min_denominator
                    abs_float_bound=(result,result)
            if is_positive:
                self._float_bound=abs_float_bound
            else:
                abs_left,abs_right=abs_float_bound
                self._float_bound=(-abs_right,-abs_left)
            return self._float_bound
        def __float__(self):
            if not self._is_possible_irrational:
                return float(self._exact_rational)
            left_float,right_float=self.float_bound()
            if left_float==right_float:
                return left_float
            infty=float('inf')
            if right_float==infty:
                infty_test=((1<<54)-1)<<970
                return (infty if self._floor>=infty_test else left_float)
            minfty=-infty
            if left_float==minfty:
                minfty_test=-(((1<<54)-1)<<970)
                return (minfty if self._ceil<=minfty_test else right_float)
        ...
        def _analyze_input_for_operator(cls,other:RealLike)->tuple[IntegerRatio|Self,bool,bool]:
            if isinstance(other,cls._family_root):
                ...
                if not possible_irrational:
                    return other._exact_rational.as_integer_ratio(),possible_rational,possible_irrational
                else:
                    return other,possible_rational,possible_irrational
        ...
        def _factory_for_operator(cls,rational_op_rational:RopR[Q],irrational_op_rational:IopR[Self],irrational_rop_rational:IopR[Self],
            ...
            def irrational_op_irrational(self:Self,other:Self)->SignFunction:
                def new_sign_func(numerator:int,denominator:int)->CompareResult:
                    ...
                    if numerator*denominator_left<=denominator*numerator_left:
                        return -1
                    numerator_right,denominator_right=next(op_iterator)
                    if numerator*denominator_right>=denominator*numerator_right:
                        return 1
                    while True:
        ...
        def _refine_generator_for_add(self,other:Self)->RefineOutput:
            ...
            while True:
                ...
                if other_L_is_updated or other_R_is_updated:
                    ...
                    other_diff_numerator=other_cross_R-other_cross_L
                    other_diff_denominator=other_L_denominator*other_R_denominator
                if self_L_is_updated:
                    self_L_is_updated=False
                if self_R_is_updated:
                    self_R_is_updated=False
                if other_L_is_updated:
                    other_L_is_updated=False
                if other_R_is_updated:
                    other_R_is_updated=False
        @staticmethod
        ...
        @classmethod
        def _generated_operator_add(cls,RR:RopR[Q],FIR:IopR[Self],G:RefineGenerator[Self],Iop:Interval_op[Q])->tuple[Operation[Self],Operation[Self]]:
            FRI=FIR
            factories=cls._factory_for_operator(RR,FIR,FRI,G)
            FII=factories[4]
            products=cls._products_for_operator(factories,Iop)
            ...
            def left_operator(self:Self,other:RealLike)->Self:
                ...
                local_cls=type(self)
                try:
                    other_obj,other_is_possible_rational,other_is_possible_irrational=local_cls._analyze_input_for_operator(other)
                except TypeError:
                    return NotImplemented
                if not other_is_possible_irrational:
                    numerator_other,denominator_other=other_obj
                    if numerator_other==0:
                        return self
                    if not self._is_possible_irrational:
                        numerator_self,denominator_self=self._exact_rational.as_integer_ratio()
                        if numerator_self==0:
                            return local_cls(other_obj,True,False)
                        ...
                        left,right=Iop(self._nearest_left,self._nearest_right,other_obj,other_obj)
                        return local_cls(new_sign_func,False,True,left,right)
                    else:
                        return UR(self,other_obj)
                    ...
                    if not self._is_possible_irrational:
                        ...
                        left,right=Iop(rational_self,rational_self,other_obj._nearest_left,other_obj._nearest_right)
                        return local_cls(new_sign_func,False,True,left,right)
                    elif not self._is_possible_rational:
                        new_sign_func=FII(self,other_obj)
                        left,right=Iop(self._nearest_left,self._nearest_right,other_obj._nearest_left,other_obj._nearest_right)
                        return local_cls(new_sign_func,True,True,left,right)
                    else:
                        return UI(self,other_obj)
                else:
                    if not self._is_possible_irrational:
                        self_rational=self._exact_rational.as_integer_ratio()
                        if self_rational[0]==0:
                            return local_cls(other_obj,True,True)
                        return RU(other_obj,self_rational)
                    elif not self._is_possible_rational:
                        return IU(self,other_obj)
                    else:
                        return UU(self,other_obj)
            def right_operator(self:Self,other:RealLike)->Self:
                local_cls=type(self)
                try:
                    init_other_obj,is_exactly_rational=local_cls._analyze_input_for_sign_function(other)
                except TypeError:
                    return NotImplemented
                if is_exactly_rational:
                    ...
                    if not self._is_possible_irrational:
                        numerator_self,denominator_self=self._exact_rational.as_integer_ratio()
                        if numerator_self==0:
                            return local_cls(init_other_obj,True,False)
                        ...
                        left,right=Iop(init_other_obj,init_other_obj,self._nearest_left,self._nearest_right)
                        return local_cls(new_sign_func,False,True,left,right)
                    else:
                        return RU(self,init_other_obj)
                else:
        ...
        def _generated_operator_sub(cls,
                                    RR:RopR[Q],FIR:IopR[Self],FRI:IopR[Self],G:RefineGenerator[Self],Iop:Interval_op[Q])->tuple[Operation[Self],Operation[Self]]:
            factories=cls._factory_for_operator(RR,FIR,FRI,G)
            FII=factories[4]
            products=cls._products_for_operator(factories,Iop)
            UR,RU,UI,IU,UU=products
            def left_operator(self:Self,other:RealLike)->Self:
                ...
                if not other_is_possible_irrational:
                    ...
                    if not self._is_possible_irrational:
                        numerator_self,denominator_self=self._exact_rational.as_integer_ratio()
                        if numerator_self==0:
                            return local_cls((-numerator_other,denominator_other),True,False)
                        result=RR(numerator_self,denominator_self,numerator_other,denominator_other)
                        return local_cls(result,True,False)
                    elif not self._is_possible_rational:
                        new_sign_func=FIR(self,numerator_other,denominator_other)
                ...
                if not other_is_possible_irrational:
                    ...
                    if not self._is_possible_rational:
                        if other_is_positive:
                            ...
                            if self_is_positive:
                                left,right=I_pos_pos(self._nearest_left,self._nearest_right,other_obj,other_obj)
                            else:
                                left,right=I_neg_pos(self._nearest_left,self._nearest_right,other_obj,other_obj)
                        else:
                            new_sign_func=FIR_neg(self,numerator_other,denominator_other)
                            if self_is_positive:
                                left,right=I_pos_neg(self._nearest_left,self._nearest_right,other_obj,other_obj)
                            else:
                                left,right=I_neg_neg(self._nearest_left,self._nearest_right,other_obj,other_obj)
                        return local_cls(new_sign_func,False,True,left,right)
                    else:
                        if self_is_positive:
                            if other_is_positive:
                                return UR_pos_pos(self,other_obj)
                            else:
                                return UR_pos_neg(self,other_obj)
                        else:
                            if other_is_positive:
                                return UR_neg_pos(self,other_obj)
                            else:
                                return UR_neg_neg(self,other_obj)
                if not self._is_possible_irrational:
                    ...
                    if not other_is_possible_rational:
                        ...
                        if self_is_positive:
                            if other_is_positive:
                                return RU_pos_pos(other_obj,rational_self)
                            else:
                                return RU_pos_neg(other_obj,rational_self)
                        else:
                            if other_is_positive:
                                return RU_neg_pos(other_obj,rational_self)
                            else:
                                return RU_neg_neg(other_obj,rational_self)
                self_is_positive=self._floor>=0
                other_is_positive=other_obj._floor>=0
                if not other_is_possible_rational:
                    if not self._is_possible_rational:
                        ...
                        if self_is_positive:
                            if other_is_positive:
                                return UI_pos_pos(self,other_obj)
                            else:
                                return UI_pos_neg(self,other_obj)
                        else:
                            if other_is_positive:
                                return UI_neg_pos(self,other_obj)
                            else:
                                return UI_neg_neg(self,other_obj)
                else:
                    if not self._is_possible_rational:
                        if self_is_positive:
                            if other_is_positive:
                                return IU_pos_pos(self,other_obj)
                            else:
                                return IU_pos_neg(self,other_obj)
                        else:
                            if other_is_positive:
                                return IU_neg_pos(self,other_obj)
                            else:
                                return IU_neg_neg(self,other_obj)
                    else:
                        if self_is_positive:
                            if other_is_positive:
                                return UU_pos_pos(self,other_obj)
                            else:
                                return UU_pos_neg(self,other_obj)
                        else:
                            if other_is_positive:
                                return UU_neg_pos(self,other_obj)
                            else:
                                return UU_neg_neg(self,other_obj)
            def right_operator(self:Self,other:RealLike)->Self:
                local_cls=type(self)
                try:
                    init_other_obj,is_exactly_rational=local_cls._analyze_input_for_sign_function(other)
                ...
                if is_exactly_rational:
                    ...
                    if not self._is_possible_rational:
                        ...
                        if other_is_positive:
                            if self_is_positive:
                                return RU_pos_pos(self,init_other_obj)
                            else:
                                return RU_pos_neg(self,init_other_obj)
                        else:
                            if self_is_positive:
                                return RU_neg_pos(self,init_other_obj)
                            else:
                                return RU_neg_neg(self,init_other_obj)
                else:
                    other_obj=local_cls.__new__(local_cls)
                    other_obj._is_called=False
                    other_obj.__init__(None,None,init_other_obj,True,True)
        ...
        def _rational_div_rational(cls,op1_numerator:int,op1_denominator:int,op2_numerator:int,op2_denominator:int)->Q:
            RaN=cls._Rational
            if op2_numerator>=0:
                result=RaN(op1_numerator*op2_denominator,op1_denominator*op2_numerator)
            else:
                result=RaN(-op1_numerator*op2_denominator,-op1_denominator*op2_numerator)
            return result
            ...
            def left_operator(self:Self,other:RealLike)->Self:
                ...
                if other_is_possible_irrational:
                    ...
                    if not other_obj._is_possible_irrational:
                        ...
                        other_is_possible_rational=other_obj._is_possible_rational
                if not other_is_possible_irrational:
                    numerator_other,denominator_other=other_obj
                    if numerator_other==0:
                        raise ZeroDivisionError("division by zero")
        ...
        def root_finding(cls,func:Callable[[int,int],RealLike],interval:tuple[RationalLike,RationalLike])->Self:
            ...
            if init_left.denominator==0 or init_right.denominator==0:
            ...
            if right_sign==0:
                return cls._convert_from_rational(init_right)
