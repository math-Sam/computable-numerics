...
def bounded_denominator_bracket(numerator:int,denominator:int,max_denominator:int)->tuple[tuple[int,int],tuple[int,int]]:
    """Return the neighboring denominator-bounded rationals bracketing n/d."""
    ...
    if left_error>right_error:
        nb=shift+1; eb=right_error; nc=shift; ec=left_error; current_left=True
    else:
        nb=shift; eb=left_error; nc=shift+1; ec=right_error; current_left=False
    while True:
        k,enew=divmod(ec,eb)
        chosen=dc+k*db
...
def nearest_bounded_denominator(numerator:int,denominator:int,max_denominator:int)->tuple[int,int]:
    left,right=bounded_denominator_bracket(numerator,denominator,max_denominator)
    if left==right:return left
    ln,ld=left; rn,rd=right
    cmp=2*numerator*ld*rd-denominator*(ln*rd+rn*ld)
